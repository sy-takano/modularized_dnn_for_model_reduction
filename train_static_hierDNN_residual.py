...
def main(num_layer, hidden_size, date_folder, gamma=None, used_module=None):
    # 静的関数を対象として階層的DNNの学習を行う

    if used_module == None:
        used_module = num_layer - 2
    ...
    model_name = 'HierDNN_residual_' + date_folder + AFFIX + '.pth'
    ...
    if used_module > 0:
        date_folder_pre_list = date_folder.rsplit('-', 1)
        date_folder_pre_list[-1] = str(int(date_folder_pre_list[-1])-1)
        date_folder_pre = date_folder_pre_list[0] + '-' + date_folder_pre_list[1]

        model_name_pre = 'HierDNN_residual_' + date_folder_pre + AFFIX + '.pth'
        model_path_pre = MODEL_FOLDER + model_name_pre
        model.load_state_dict(torch.load(model_path_pre, map_location=torch.device('cpu')))
        # for i in range(used_module):
        #     model.fc_list[i].weight.requires_grad = False
        #     model.fc_list[i].bias.requires_grad = False
        #     # model.fc_list[used_module - 1].weight.requires_grad = False
        #     # model.fc_list[used_module - 1].bias.requires_grad = False
    ...
    #     w_pre.append(model_pre.fc_list[i].weight.detach())
    #     b_pre.append(model_pre.fc_list[i].bias.detach())
    ...
    # EPOCHS = round(20000 / (NUM_LAYER-1))
    EPOCHS = 20000
    ...
    tic()   # 学習時間測定開始
    loss_history, loss_history_termwise, epoch_history, grad_history, fit_val_history = train_signal_regularization(model, u, y, num_layer=used_module+2, regularization_parameter=gamma, path_histogram=f'{FIGURE_FOLDER}grad_hist', u_val=u_val, y_val=y_val, epochs=EPOCHS, device=device)
    toc()   # 学習時間測定終了
    ...
    yhat_val, intermediate_y_hat_val = model(u_val, num_used_layers=used_module+2) # yhat: 全層使った出力，intermediate_y_hat: サブ出力
    ...
    # for i in range(NUM_LAYER - 1):
    #     print(w_pre[i] == model.fc_list[i].weight)
    #     print(b_pre[i] == model.fc_list[i].bias)
...
if __name__ == '__main__':
    for trial in range(6, 7):
        for i in range(2, 6):
            print(i)
            date_folder = '2023-01-18'+'-'+str(trial)+'-'+str(i)
            # date = date.today().strftime('%Y-%m-%d')+'-'+str(trial)+'-'+str(i)
            main(num_layer=5, hidden_size=64, date_folder=date_folder, used_module = i-2)

        # for i in range(2, 18):
        #     print(i)
        #     date_folder = '2023-01-18'+'-'+str(trial)+'-'+str(i)
        #     # date = date.today().strftime('%Y-%m-%d')+'-'+str(trial)+'-'+str(i)
        #     main(num_layer=17, hidden_size=23, date_folder=date_folder, used_module = i-2)
    ...
    # main(num_layer=5, hidden_size=64, date_folder=date.today().strftime('%Y-%m-%d'))
