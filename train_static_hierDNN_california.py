...
def main(num_layer, hidden_size, date_folder, gamma=None):
    ...
    yhat, intermediate_y_hat = model(u) # yhat: 全層使った出力，intermediate_y_hat: サブ出力
    ...
    # サブ出力を真の関数と比較
    plt.figure()
    plt.plot(u.detach().cpu().numpy(), yhat.detach().cpu().numpy(), label='y')
    for i in range(NUM_LAYER-1):
        plt.plot(u.detach().cpu().numpy(), intermediate_y_hat[i].detach().cpu().numpy(), label=f'o_{i+1}')
    plt.plot(u.detach().cpu().numpy(), 0.166*torch.ones_like(u).detach().cpu().numpy(), '-.k', label='BLA')
    plt.grid()
    plt.legend()
    plt.xlabel('u')
    plt.ylabel('y')
...
if __name__ == '__main__':
    for i in range(5 - 1):
        ...
        gamma[-1-i] = 1.
        print(gamma)
        main(num_layer=5, hidden_size=64, date_folder=date.today().strftime('%Y-%m-%d')+'-'+str(5-i), gamma=gamma)

    for i in range(17 - 1):
        gamma = [0. for i in range(17 - 1)]
        gamma[-1-i] = 1.
        print(gamma)
        main(num_layer=17, hidden_size=23, date_folder=date.today().strftime('%Y-%m-%d')+'-'+str(17-i), gamma=gamma)
