...
# 階層的な線形状態空間モデルの学習を行う
# 事前にMATLABのssestで推定したパラメータを初期値とする
...
name = date.today().strftime('%Y-%m-%d') + '-' + str(NUM_LAYER) + '_4_oscil100_3000_losstime10_modalstart'
...
model.lti_list[0].B.weight = nn.Parameter(B_ini[0:2, :])
model.lti_list[0].D.weight = nn.Parameter(D_ini)

for i in range(NUM_LAYER):
    model.lti_list[i].A.weight = nn.Parameter(A_ini[2*i:2*(i+1), 2*i:2*(i+1)])
    model.lti_list[i].C.weight = nn.Parameter(C_ini[:, 2*i:2*(i+1)])
