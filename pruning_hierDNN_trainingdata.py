...
def main(num_layer, hidden_size, date):
    # 階層的DNNとFNNを枝刈りして，学習データに対する出力を比較

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(device)

    date = f'{date}-{num_layer}-{hidden_size}'
    FIGURE_FOLDER = './figures/HierDNN/' + date + '-california/'
    model_path = './TrainedModels/HierDNN_california_' + date + '-california.pth'    # 学習済みの階層的DNNのパス
    # model_path_DNN = './TrainedModels/DNN_california_2022-04-18-5-64.pth'      # 学習済みのFNNのパス
    # model_path_DNN = './TrainedModels/DNN_sinc_' + date + '.pth'      # 学習済みのFNNのパス

    outfile_path = FIGURE_FOLDER + 'accuracy_train.csv'   # 適合率の出力ファイルパス
    ...
    for i in range(NUM_LAYER-1):
        ...
        # plt.ylabel('y')
        # plt.legend()
        # plt.savefig(f'{FIGURE_FOLDER}pruned/fig_training_{i}.png')
